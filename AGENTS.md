...
Use `swift build` to compile the package targets. Use `swift run mac-cleaner` for the interactive CLI, `swift run mac-cleaner scan --json` for machine-readable scan output, and `swift run mac-cleaner clean --dry-run --dev-caches --yes` to preview deletions safely. Run `swift test` to execute the Swift Testing suites; note that `Tests/MacCleanerAppTests/` is not part of the SwiftPM test target — full/CI verification also needs `xcodebuild test -project MacCleaner.xcodeproj -scheme MacCleanerApp -destination 'platform=macOS'`. When `project.yml` changes, regenerate the app project with `xcodegen generate`, then open `MacCleaner.xcodeproj` in Xcode to run the macOS app.
