...
基础扫描和清理均在本机进行。若主动配置 DeepSeek API Key 并同意隐私说明，AI 分析会发送已选项目的路径、大小、时间、类型与来源标签，或已选进程的基本运行信息；不会发送文件内容、环境变量或完整命令行。API Key 保存在当前 Mac 的应用偏好中，不会写入 macOS 钥匙串，因此不会触发钥匙串授权；也请勿将本机账户交给不受信任的人使用。
